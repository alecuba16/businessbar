...
# Resolve dependencies first so the checkout is available for patching.
swift package resolve

# Patch KeyboardShortcuts to avoid fatal `Bundle.module` trap when the
# resource bundle is not at the .app root (SPM generates a path that only
# works when running as a bare SPM executable, not inside a .app bundle).
# This is a known macOS 26+ issue: the SPM-generated Bundle.module accessor
# uses Bundle.main.bundleURL.appendingPathComponent("name.bundle") which
# resolves to the .app root — but bundles live in Contents/Resources/.
# See: https://github.com/sindresorhus/KeyboardShortcuts/issues/229
patch_keyboard_shortcuts() {
    local util_path="$PROJECT_DIR/.build/checkouts/KeyboardShortcuts/Sources/KeyboardShortcuts/Utilities.swift"
    if [ ! -f "$util_path" ]; then
        echo "  ⚠ KeyboardShortcuts Utilities.swift not found — skipping Bundle.module patch"
        return
    fi

    # Already patched?
    if grep -q "keyboardShortcutsSafeBundle" "$util_path" 2>/dev/null; then
        echo "  ✓ KeyboardShortcuts already patched (safe bundle lookup)"
        return
    fi

    chmod +w "$util_path"

    # Replace .module with .keyboardShortcutsSafeBundle in NSLocalizedString call
...
private extension Bundle {
    /// Safe lookup that avoids the fatal trap in the autogenerated
    /// `Bundle.module` when the resource bundle is not at the .app root.
    /// Uses `url(forResource:withExtension:)` which correctly searches
...
# Insert the safe bundle lookup before the "extension Data {" block.
marker = "\nextension Data {"
...
PY
    ...
    echo "  ✓ KeyboardShortcuts patched (safe bundle lookup via url(forResource:withExtension:))"
}
...
# 4b. Sign SPM resource bundles in Contents/Resources/
#     These must be signed BEFORE the main app so the inside-out order is
#     correct.  Unsigned bundles inside a signed .app may be rejected by
#     macOS at runtime, causing Bundle.module to return nil and fatalError.
for bundle_dir in "$APP/Contents/Resources"/*.bundle; do
    [ -d "$bundle_dir" ] || continue
    codesign --force --sign - "$bundle_dir" 2>/dev/null || true
done
echo "  ✓ Resource bundles signed"
